...
所有会创建、保存、更新数据的提交按钮，必须防止重复点击导致重复提交。

要求：

- 页面数据中必须有明确的提交中状态，例如 `saving` / `isSubmitting` / `isLoggingIn`
- 提交方法开始时先判断提交中状态，若正在提交则直接 `return`
- 发起请求前设置提交中状态为 `true`
- WXML 按钮必须绑定 `loading` 和 `disabled`
- 请求失败时恢复提交中状态
- 请求成功并即将返回上一页/跳转时，可以不恢复，避免用户在跳转前再次点击
