...
1. 标题和年份
2. 本月统计卡片
3. 主力球拍卡片（点击进入球拍详情；无主力拍时展示设置引导）
4. 主力球鞋卡片（点击进入球鞋详情；无主力鞋时展示设置引导）
5. 近几场手感趋势
6. 最近打球卡片
7. 底部 `+` 快速新增按钮
...
列表页右上角的 ⚙︎ 可打开“列表展示字段”面板，勾选后列表卡片将显示对应信息，选择仅保存在本机：

| 字段 | 说明 |
|---|---|
| 打球时间点 | 勾选时按当前逻辑显示日期 + `HH:mm`；不勾选时时间只精确到日期 |
| 时长 | 显示 `durationMinutes` 分钟 |
| 场地 / 搭档 / 球拍 / 球鞋 / 费用 / 备注 | 对应字段非空时展示 |

新增字段默认勾选；旧版本已保存的字段偏好会自动补上“打球时间点”，保证默认显示逻辑不变。
